...
3. Proxy Base URL input is moved to the Settings pane (admin-only concern, persisted in localStorage)
...
10. `updateWebhookWarning()` shows proxy status badge when token exists, register button when conditions are met
