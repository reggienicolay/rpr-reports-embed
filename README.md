...
**v1.5.0** — Phase 3: Transparent proxy with server-side formatters. Agents self-serve via the generator — webhook URLs are auto-registered with the Cloudflare Worker on input blur, no API keys needed. `POST /api/config` is public for self-service; management endpoints remain authenticated. Worker detects destination (Slack, Discord, ntfy, SimplePush, Pushover) and formats payloads server-side (Block Kit, embeds, headers, etc.); unrecognized URLs get raw JSON passthrough. Removed all proxy jargon from agent-facing UI. All 10 integrations verified end-to-end.

**Help page** — Added an on-page **Install Guide & FAQ** below the generator (`help.css` / `help.js`, scoped under `.rpr-help`; CSP-safe external JS): sticky table of contents with active-section highlighting, platform install tabs, copy-to-clipboard, and live FAQ search. Content verified against actual widget behavior (real lead-delivery methods, `data-*` attributes, `data-modal-trigger` CSS selector, generator auto-save, `https://` report-URL acceptance). A parallel native **Avada Builder** build of the same help content lives in `avada/` for embedding on `blog.narrpr.com`. No widget runtime changes.
